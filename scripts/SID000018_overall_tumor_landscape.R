...
modelPurity <- function(dataMatrix, outstatsFileName, outheatmapFileName){
  ...
  toCorrelateAll <- as.vector(c('totalSNVsCalled',dataTypes[c('Purity','relTumorDistance','OldhamDecon','CancerSEA')] %>% unlist))
  allPairsPurity <- cbind(rbind(c("purity"),toCorrelateAll))#combn(toCorrelateAll, 2)
  ...
  col_breaks = c(seq(min(mofInterest, na.rm=T),break1,length=100), # for yellow (~1st quartile)
                 seq(break1+.001,break2-.001,length=100), # for black (~within 2nd and 3rd quartiles)
                 seq(break2,max(mofInterest, na.rm=T),length=100)) # for blue (~4th quartile)
...
plotFinalPurity(allPairsPurityResultsIDHWT, allPairsPurityResultsIDHMut){
  merged <- merge(allPairsPurityResultsIDHWT, allPairsPurityResultsIDHMut, by='b')
  mergedSubset <- merged[,c('b','estimate.x','isSignificant.x','estimate.y','isSignificant.y')]
  mofInterest <- mergedSubset[which(mergedSubset$isSignificant.IDHWT ==1 | mergedSubset$isSignificant.IDHMut ==1),c('estimate.IDHWT','estimate.IDHMut')]
  rownames(mofInterest) <- mergedSubset[which(mergedSubset$isSignificant.IDHWT ==1 | mergedSubset$isSignificant.IDHMut ==1),]$b
  colnames(mergedSubset) <- c('b','estimate.IDHWT','isSignificant.IDHWT','estimate.IDHMut','isSignificant.IDHMut')
  my_palette <- colorRampPalette(c("mediumblue", "grey", "red"))(n = 299)
  colOrder <- order(rowMeans(mofInterest))
  mofInterest <- t(mofInterest)[,colOrder]
  break1 <- -.1
  break2 <- .1
  pdf(paste0(outFolder, 'final_IDHWT_IDHMut_comparision_heatmap_estimate.pdf'), width=10, height=10)
  my_palette <- colorRampPalette(c("mediumblue", "grey", "orangered4"))(n = 299)
  col_breaks = c(seq(min(mofInterest, na.rm=T),break1,length=100), # for yellow (~1st quartile)
                 seq(break1+.001,break2-.001,length=100), # for black (~within 2nd and 3rd quartiles)
                 seq(break2,max(mofInterest, na.rm=T),length=100)) # for blue (~4th quartile)
...
modelDistances <- function(dataMatrix, allPairsPurityResults, outstatsFileName, outheatmapFileName){
  ...
  toCorrelateAll <- as.vector(c('totalSNVsCalled',dataTypes[c('Purity','relTumorDistance','OldhamDecon','CancerSEA')] %>% unlist))
  allPairs <- cbind(rbind(c("DistCentroid"),toCorrelateAll),rbind(c("DistPeriph"),toCorrelateAll),rbind(c("DistVR"),toCorrelateAll))#combn(toCorrelateAll, 2)
  allPairsResults <- data.frame(a=character(), b=character(),estimate=numeric(), pvalue=numeric())
  for (i in 1:ncol(allPairs)){
    ...
    if (!(a == b | b == 'purity')){
      ...
      #                         label.x.npc = 'right', label.y.npc = 0.4, size = 3))
      # dev.off()
      if (allPairsPurityResults[which(allPairsPurityResults$b == b),]$isSignificant == 1){
  ...
  my_palette <- colorRampPalette(c("mediumblue", "grey", "orangered4"))(n = 299)
  col_breaks = c(seq(min(mofInterest, na.rm=T),break1,length=100), # for yellow (~1st quartile)
                 seq(break1+.0001,break2-.0001,length=100), # for black (~within 2nd and 3rd quartiles)
                 seq(break2,max(mofInterest, na.rm=T),length=100)) # for blue (~4th quartile)
  hm.parameters <- list(mofInterest,cellwidth=10, cellheight=10, fontsize=10, border_col=NA, cluster_rows = F, cluster_cols = F, col= my_palette, breaks = col_breaks, na_col='white')
  do.call("pheatmap", hm.parameters)
  dev.off()
...
allPairsPurityResults <- modelPurity(mtmp, paste0(tag,'_purity_vs_signatures_lmer_with_re_padjBH_stats.txt'),paste0(tag,'SID000018_purity_vs_signatures_lmer_with_re_padjBH_heatmap.pdf'))

# repeats above purity for idh-wt TERT tumors only
allPairsPurityResultsIDHWT <- modelPurity(mtmp[which(mtmp$IDH_Mut==0 & mtmp$TERT==1),], 'SID000018_IDHWTTERTonly_purity_vs_signatures_lmer_with_re_padjBH_stats.txt','SID000018_IDHWTTERT_purity_vs_signatures_lmer_with_re_padjBH_heatmap.pdf')

# repeats above purity for idh-mut tumors only
allPairsPurityResultsIDHMut <- modelPurity(mtmp[which(mtmp$IDH_Mut==1),], 'SID000018_IDHMutonly_purity_vs_signatures_lmer_with_re_padjBH_stats.txt','SID000018_IDHMutonly_purity_vs_signatures_lmer_with_re_padjBH_heatmap.pdf')
...
# for each distance metric, fit a linear model to see its relationship to each cell type/state signature
#  that controls for repeat measures (allows for different intercept for each patient)
modelDistances(mtmp, allPairsPurityResults, 'SID000018_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_stats.txt','SID000018_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_heatmap.pdf')

#repeats above distances for idh-wt TERT tumors only
modelDistances(mtmp[which(mtmp$IDH_Mut==0 & mtmp$TERT==1),], allPairsPurityResultsIDHWT, 'SID000018_IDHWTTERTonly_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_stats.txt','SID000018_IDHWTTERTonly_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_heatmap.pdf')

#repeats above distances for idh-wt TERT tumors only
modelDistances(mtmp[which(mtmp$IDH_Mut==1),], allPairsPurityResultsIDHMut, 'SID000018_IDHMutonly_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_stats.txt','SID000018_IDHMutonly_distance_metrics_vs_signatures_lmer_opt_purity_with_re_padjBH_heatmap.pdf')
